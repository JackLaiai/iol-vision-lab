...
window.EvidenceRouting = {
 ...
 matches(record, product) { return product.product_key ? record.product?.product_key === product.product_key && (record.product?.model_number === product.model_number || (record.evidence_scope === "product family" && record.product?.catalog_models?.includes(product.model_number))) : record.product?.model_number === product.model_number; },
